  ...
  var promise = new Promise(function (resolve, reject) {
    transporter.sendMail(mailOptions, function (err, info) {
      if (err) {
        ...
        db.sg.update({_id: {$in: ids}}, {$set: {sent: true}}, {multi: true}).then(function (result) {
          resolve(result);
        }, function (err) {
          resolve(err);
        });
...
readfile('./templates/item.hbs', {encoding: 'utf8'}).then(function (result) {
  ...
  db.sg.find({sent: {$ne: true}}).sort({timestamp: -1}).then(function (items) {
    if (items.length > 0) {
